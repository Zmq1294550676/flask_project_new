...
class Mongo_service(object):
    ...
    def __init__(self, collection, new_name):
        if new_name is None:
            ...
            self.qs = self.switch_table_and_return_query_set(collection, new_name)
            self.qs_name = new_name
        self.old_collection = collection
    
    def switch_table_and_return_query_set(self, collection, new_name):
